...
class studentController extends mainModel
{
	...
	public function registrarEstudianteControlador()
	{
		...
		$acudiente = $this->cleanQuery($_POST['acudiente_id']);
		...
		if ($identificacion == "" || $nombre == "" || $apellido == "" || $usuario == "" || $rol == "" || $direccion == "" || $telefono == "" || $acudiente == "" || $grado == "" || $clave1 == "" || $clave2 == "") {
			$alerta = [
				"tipo" => "simple",
		...
		$estudiante_datos_reg = [
			...
			[
				"campo_nombre" => "acudiente_id",
				"campo_marcador" => ":Acudiente",
				"campo_valor" => $acudiente
			],
			[
	...
	public function listarEstudianteControlador($pagina, $registros, $url, $busqueda)
	{
		...
		if (isset($busqueda) && $busqueda != "") {
			$consulta_datos = "SELECT *, 
                acudientes.acudiente_nombre, 
                acudientes.acudiente_apellido, 
                grados.grado_nombre 
				FROM estudiantes 
				LEFT JOIN 
                acudientes ON estudiantes.acudiente_id = acudientes.acudiente_id
                LEFT JOIN 
                grados ON estudiantes.grado_id = grados.grado_id 
				WHERE ((estudiante_id!='" . $_SESSION['id'] . "' AND estudiante_id!='0') AND (estudiante_nombre LIKE '%$busqueda%' OR estudiante_apellido LIKE '%$busqueda%' OR estudiante_email LIKE '%$busqueda%' OR estudiante_usuario LIKE '%$busqueda%')) ORDER BY estudiante_nombre ASC LIMIT $inicio,$registros";
			...
			$consulta_datos = "SELECT *, 
                acudientes.acudiente_nombre, 
                acudientes.acudiente_apellido, 
                grados.grado_nombre  FROM estudiantes
				LEFT JOIN 
                acudientes ON estudiantes.acudiente_id = acudientes.acudiente_id
                LEFT JOIN 
                grados ON estudiantes.grado_id = grados.grado_id
				WHERE estudiante_id!='" . $_SESSION['id'] . "' AND estudiante_id!='0' ORDER BY estudiante_nombre ASC LIMIT $inicio,$registros";
		        ...
		        <table class="table table-bordered table-hover">
		            <thead>
		                <tr class="table-primary">
							...
							<th class="col text-center">Acudiente</th>
	...
	public function actualizarEstudianteControlador()
	{
		...
		# Verificando campos obligatorios #
		if ($nombre == "" || $apellido == "" || $usuario == "" || $acudiente == "" || $grado == "")  {
			$alerta = [
				"tipo" => "simple",
				"titulo" => "Ocurrió un error inesperado",
				"texto" => "No has llenado todos los campos que son obligatorios",
