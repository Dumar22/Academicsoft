   ...
   namespace controllers; 
	...
	class materiasController extends mainModel{
		...
		public function eliminarMateriaControlador() {
			...
			// Verificar si es el materia principal
			if ($id == 1) {
				$alerta = [
					"tipo" => "simple",
					"titulo" => "Ocurrió un error inesperado",
					"texto" => "No podemos eliminar el materia principal del sistema",
					"icono" => "error"
				];
				return json_encode($alerta);
			}
			...
			if ($datos->rowCount() <= 0) {
				$alerta = [
					"tipo" => "simple",
					"titulo" => "Ocurrió un error inesperado",
					"texto" => "No hemos encontrado el materia en el sistema",
					"icono" => "error"
				];
				return json_encode($alerta);
			}
		...
		public function actualizarMateriaControlador(){
			...
			# Verificando materia #
		    $datos=$this->executeQuery("SELECT * FROM materiaes WHERE materia_id='$id'");
		    if($datos->rowCount()<=0){
		        $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
